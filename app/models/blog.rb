...
#  has n, :comments
  ...
  def self.last( opts = {} ) first( default_options.merge(opts).merge( :conditions => ['published_at IS NOT NULL'] ) ); end
  ...
  private
    def self.default_options() { :order => [:published_at.desc] }; end

    def self.count_for( all, category_id = nil )
